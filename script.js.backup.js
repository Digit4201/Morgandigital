...
function initUrgencyCountdown() {
    // Date de fin de l'offre (31 août 2025 à 23:59)
    ...
    function updateCountdown() {
        ...
        if (distance > 0) {
            ...
            // Mise à jour du countdown dans la bannière
            ...
            if (days === 0) {
                document.querySelectorAll('.urgency-countdown, .urgency-highlight').forEach(el => {
                    ...
                    el.style.animation = 'urgencyBlink 0.5s infinite';
                });
            }
        } else {
            // L'offre est expirée
            document.querySelectorAll('.urgency-banner, .urgency-badge').forEach(el => {
                el.style.display = 'none';
            });
            
            // Restaurer les prix normaux
            document.querySelectorAll('.price-current').forEach((el, index) => {
                const originalPrices = ['€599', '€1199', '€1999'];
            ...
            document.querySelectorAll('.pricing-btn').forEach(btn => {
                btn.textContent = btn.textContent.replace(/🚀|⚡|👑/, '').replace('€399', '€599').replace('€799', '€1199').replace('€1299', '€1999');
            });
        }
    }
...
function initUrgencyBanner() {
    ...
    if (closeBtn && banner) {
        ...
        if (localStorage.getItem('urgencyBannerClosed') === 'true') {
            // Ne pas masquer complètement, juste réduire la fréquence d'affichage
            const lastClosed = localStorage.getItem('urgencyBannerClosedTime') || 0;
            const now = Date.now();
    ...
    // Animation d'apparition après 3 secondes pour attirer l'attention
    setTimeout(() => {
        if (banner && !banner.classList.contains('hidden')) {
            banner.style.animation = 'urgencySlideDown 0.5s ease-out';
        }
    }, 3000);
}

// Animation CSS pour la bannière
const urgencyAnimationCSS = `
...
styleSheet.textContent = urgencyAnimationCSS;
document.head.appendChild(styleSheet);

// Fonction pour simuler la scarcité (nombre de places restantes)
function initScarcityIndicators() {
    const scarcityData = {
        starter: Math.floor(Math.random() * 5) + 2, // 2-6 places
        professional: Math.floor(Math.random() * 3) + 1, // 1-3 places  
        enterprise: Math.floor(Math.random() * 2) + 1 // 1-2 places
    };
    
    // Ajouter les indicateurs de scarcité
    const pricingCards = document.querySelectorAll('.pricing-card');
    pricingCards.forEach((card, index) => {
        const types = ['starter', 'professional', 'enterprise'];
        const type = types[index];
        const remaining = scarcityData[type];
        
        const scarcityEl = document.createElement('div');
        scarcityEl.className = 'scarcity-indicator';
        scarcityEl.innerHTML = `
            <span class="scarcity-icon">⚠️</span>
            Plus que <strong>${remaining} places</strong> à ce prix !
        `;
        
        const priceDiv = card.querySelector('.pricing-price');
        if (priceDiv) {
            priceDiv.appendChild(scarcityEl);
        }
    });
}

// Démarrer les indicateurs de scarcité après un court délai
setTimeout(initScarcityIndicators, 2000);
...
function initFloatingOffer() {
    ...
    const claimBtn = floatingOffer.querySelector('.floating-btn-claim');
    
    if (!floatingOffer) return;
    
    let offerTimer = 15 * 60; // 15 minutes en secondes
    let timerInterval;
    
    // Afficher le banner immédiatement pour debug, puis après 2 secondes
    console.log('Floating offer initialized');
    
    // Debug: afficher immédiatement
    setTimeout(() => {
        // Vérifier si l'offre n'a pas déjà été fermée
        if (localStorage.getItem('floatingOfferClosed') !== 'true') {
            console.log('Showing floating offer');
            floatingOffer.classList.add('show');
            floatingOffer.style.display = 'block'; // Force display
            startOfferTimer();
        }
    }, 500); // Réduit à 500ms pour test
    
    // Fonction pour démarrer le timer de 15 minutes
    function startOfferTimer() {
        ...
        timerInterval = setInterval(() => {
            const minutes = Math.floor(offerTimer / 60);
            ...
            // Quand le timer arrive à 0, masquer l'offre
            if (offerTimer < 0) {
                clearInterval(timerInterval);
                floatingOffer.classList.add('hide');
                setTimeout(() => {
                    floatingOffer.style.display = 'none';
                }, 500);
            }
            
            // Animation d'urgence quand il reste moins de 2 minutes
            if (offerTimer < 120) {
                floatingOffer.style.animation = 'floatingGlow 1s ease-in-out infinite, urgencyShake 0.5s infinite';
            }
        }, 1000);
    }
    ...
    if (closeBtn) {
        closeBtn.addEventListener('click', () => {
            ...
            localStorage.setItem('floatingOfferClosedTime', Date.now().toString());
    ...
    // Action du bouton "J'en profite"
    if (claimBtn) {
        claimBtn.addEventListener('click', () => {
            // Copier le code dans le presse-papier
            navigator.clipboard.writeText('SAVE50').then(() => {
                // Feedback visuel
                ...
                alert('Code promo: SAVE50');
            });
        });
    }
    
    // Réafficher l'offre après 1 heure si elle a été fermée
    const closedTime = localStorage.getItem('floatingOfferClosedTime');
    if (closedTime && localStorage.getItem('floatingOfferClosed') === 'true') {
        const timeDiff = Date.now() - parseInt(closedTime);
        if (timeDiff > 3600000) { // 1 heure
            localStorage.removeItem('floatingOfferClosed');
            localStorage.removeItem('floatingOfferClosedTime');
        }
    }
    
    // Animation d'entrée plus tard si l'utilisateur scroll
    let hasScrolled = false;
    window.addEventListener('scroll', () => {
        if (!hasScrolled && window.scrollY > 300) {
            hasScrolled = true;
            // Ajouter une petite animation d'attention
            floatingOffer.style.transform = 'translateX(0) scale(1.05)';
            setTimeout(() => {
                floatingOffer.style.transform = 'translateX(0) scale(1)';
            }, 300);
        }
    });
}
...
// ========================================
// DARK MODE TOGGLE (ORIGINAL SYSTEM)
// ======================================== 
